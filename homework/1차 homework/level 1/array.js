...
var arr3 = ['오승재', 1, 2, 3, null, {name: 'osj', age: 23}];
console.log(arr3);
...
// 2-2, push, pop -> js array에서 push pop은 스택에서처럼 동작하는 듯!
arr.push('new item');
console.log('arr push: ', arr);
arr.pop();
console.log('arr pop: ', arr);

// 2-3 shift, unshift -> unshift 배열의 맨 앞에 추가, shift 배열 맨 앞에 값 제거!
...
// 2-5 indexOf
console.log('arr.indexOf(4): ', arr.indexOf(4)); // 4가 있는 인덱스 3 반환
console.log('arr.indexOf(100): ', arr.indexOf(100)); // 100 없기에 -1 반환

// 2-6 concat 배열을 합쳐줌
var arr1 = [1, 2, 3];
var arr2 = [4, 5, 6];
var concatArr = arr1.concat(arr2);
console.log('arr.concat(arr2): ', concatArr);

// 2-7 join
var location = ["서울", "대전", "대구", "부산"];
console.log(location.join('-> ')); // 각 요소 사이에 '-> '추가해서 합친다.
...
// 출처: https://kim-solshar.tistory.com/58 [김솔샤르의 인사이트]

// 2-10 filter는 배열 요소 전체를 대상으로 조건을 걸어서 그 조건을 충족하는 결과를 새로운 배열을 반환해줌.
...
// 2-11 map은 배열 요소 전체를 대상으로 함수를 호출하고, 그 결과를 새로운 배열을 반환할때 주로 사용
...
var serverPart = ["오승재", "석영현", "강준우", "송정우", "신지혜", "이영은", "이진호", "최영훈"];
let serverIndexStr = "서버파트 여러분 번호 한번 세겠습니다. '";
let serverPartMemberNameStr = "서버파트 여러분 이름 한번씩만 불러주세요~ '";

for(let item in serverPart) { // 이건 인덱스
    serverIndexStr += (Number(item)+1) + '! ';
}
console.log(serverIndexStr);

for(let item of serverPart) { // 이건 안에 요소
