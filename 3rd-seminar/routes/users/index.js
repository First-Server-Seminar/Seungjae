const encrypting = require('../../modules/encrypting');
...
router.post('/signup', (req, res) => {
  ...
  if (!id || !password) { //2. request data 확인하기, id 또는 password data가 없다면 NullValue 반환
  ...
  if (userIndex != -1) { //3. 존재하는 아이디인지 확인하기. 이미 존재하는 아이디면 ALREADY ID 반환
    console.log("존재하는 ID 입니다.");
  ...
  encrypting.encrypting(usersDB, id, password);
...
router.post('/signin', (req, res) => {
  ...
  if (userIndex === -1) { //3. 존재하는 아이디인지 확인하기. 존재하지 않는 아이디면 NO USER 반환
